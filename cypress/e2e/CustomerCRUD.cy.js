...
describe("Customer CRUD Functionality", () => {
  ...
  const phone = "+97797" + faker.string.numeric(8);
  const email = faker.internet.email();

  const editedFirstName = "Sarah";
  const editedLastName = "Stha";
  ...
  it("Verify Customer can be added with Valid details", () => {
    ...
    cy.contains('[role="option"]', "Poush").click();

    cy.get('button[aria-label="Choose the Nepali year"]').click();

    cy.contains('[role="option"]', "2059").click();

    cy.contains("button", /^3$/).click();

    cy.get(customer.address).type("Ktm, Nepal");
  ...
  it("Verify customer edit functionality", () => {
    ...
    cy.contains("Sarah Stha").should("be.visible");
  });

  it("Verify customer can be deleted", () => {
    cy.xpath(customer.customerpage).click();

    cy.xpath(customer.searchcustomer).clear().type("Sarah");
    ...
    cy.contains("Sarah Stha").should("not.exist");
  });
});